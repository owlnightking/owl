...
mobile 端 `apps/mobile-web/src/pages/SampleListPage.tsx`（web 端：筛选条件网格 + 操作行 + 多选表格 +
新增/编辑/详情右侧抽屉 + 骨架屏 + 固定操作列；移动端为卡片列表 + 加载更多）。
...
`http://localhost:5172/admin/sample-list`（管理台菜单「样板页」）或 `http://localhost:5173/sample-list`（mobile）。
