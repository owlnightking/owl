...
export class RecognitionService {
  ...
  async approve(id: number, approverId: number): Promise<void> {
    ...
    if (item.badgeId) {
      ...
      if (badge && badge.coinReward > 0) {
        await this.coinRepo.addBalance(
          item.receiverId,
          badge.coinReward,
          "recognition",
          String(id),
  ...
  async getLevel(userId: number): Promise<{ level: number; exp: number; nextLevelExp: number }> {
    const createdAt = await this.recognitionRepo.getUserCreatedAt(userId);
    if (!createdAt) return { level: 1, exp: 0, nextLevelExp: 1000 };

    const years = new Date().getFullYear() - createdAt.getFullYear();
    const level = Math.max(1, years);
    const exp = Math.max(0, await this.recognitionRepo.getRecognitionExp(userId));
    return { level, exp, nextLevelExp: level * 1000 };
  }
}
