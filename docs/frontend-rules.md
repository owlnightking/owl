...
>
> **不给「刷新」按钮**：列表页一律不放手动刷新入口。重新取数由「搜索 / 重置 / 翻页 / 切换每页条数 /
> 增删改保存后自动重新请求」承担，独立刷新按钮是多余入口（同一个动作两条路径，还会被误当成数据同步手段）。
> 只读列表（无新增 / 批量 / 行内动作、也没有页面级 tab）**不设操作行**，不要留一行只放刷新按钮。
> 注意「重置」和「刷新」不是一回事：筛选区最后一行最右的「重置」是清空筛选条件并回到第 1 页，
> 它用 `IconRefresh` 图标是合规的，不要连它一起删。
>
